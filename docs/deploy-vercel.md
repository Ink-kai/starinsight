...
- 生产和 Vercel Preview 必须使用 `REPORT_STORE=supabase`；`REPORT_STORE=json` 仅保留为本地开发模式。
...
| `REPORT_STORE` | `supabase` | 预发布 / 生产报告存储必须使用 Supabase；`json` 只用于本地开发。 |
...
该脚本允许 `REPORT_STORE=supabase` 或 `REPORT_STORE=json`，其中 `json` 只用于本地开发；Vercel Preview / Production 必须配置 `REPORT_STORE=supabase`。脚本还会要求 `ADMIN_TOKEN` 至少 32 个字符，以便提前发现不适合预发布 / 生产部署的配置。
...
4. 在 Supabase SQL Editor 执行 `supabase/migrations/001_create_reports.sql`，确认 `reports` 表已创建。
...
- [ ] Supabase migration `supabase/migrations/001_create_reports.sql` 是否已执行。
...
`REPORT_STORE=supabase` 时，服务端运行时会使用 `SupabaseReportStore` 写入 `public.reports` 表；`REPORT_STORE=json` 只用于本地开发并写入 `data/reports`。上线前请确认 `reports` 表包含 `id`、`access_token`、`birth_info`、`chart_data`、`ai_summary`、`ai_highlights`、`ai_full_report`、`status`、`metadata`、`created_at`、`updated_at`。
