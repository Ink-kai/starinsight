...
const ALLOWED_REPORT_STORES = ['supabase', 'json'];
const PREVIEW_REPORT_STORE = 'supabase';
...
if (!isMissing('REPORT_STORE')) {
  if (!ALLOWED_REPORT_STORES.includes(process.env.REPORT_STORE)) {
    errors.push(`REPORT_STORE must be one of: ${ALLOWED_REPORT_STORES.join(', ')}.`);
  }

  if (process.env.REPORT_STORE === 'json') {
    warnings.push('REPORT_STORE=json is intended for local development only; Vercel Preview/Production should use REPORT_STORE=supabase.');
  }
...
console.log(`REPORT_STORE allowed values: ${ALLOWED_REPORT_STORES.join(', ')}`);
console.log(`Vercel Preview/Production expected value: ${PREVIEW_REPORT_STORE}`);
